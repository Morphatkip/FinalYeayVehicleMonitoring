...
obj = None
frameTimeStamp = []
stateLPInitial = set(["AP", "AR", "AS", "BR", "CG", "GA", "GJ", "HR", "HP", "JH", "KA", "KL", "MP", "MH", "MN", "ML",
                     "MZ", "NL", "OD", "PB", "RJ", "SK", "TN", "TS", "TR", "UP", "UK", "WB", "AN", "CH", "DN", "DD", "DL", "JK", "LD", "PY"])
    ...
    for bbox in bboxes:  # iterating through bboxes of first detection
        if bbox[4] == 2 and bbox[5] >= 0.9:
            try:
                ...
                ############################
                '''
                image[top_left[1]:top_left[1] + crop_height,
                      top_left[0]:top_left[0]+crop_width] = car_img
                cv2.imwrite(output_path + "data/" +
                            '{}' .format(filename), image)
                print(filename, logo, lp_text, colour)
                frameNo = filename[5:]
                frameNo = int(frameNo[:-4])
                obj.writerow(("this is the file name" + filename, "logo:"+logo, "lp text:" + lp_text,
                             "colour:" + colour, frameTimeStamp[frameNo]))
...
def get_image_data(filepath, filename):
    ...
    obj.writerow(("Frame No.", "Manufacturer",
                 "License Plate", "Colour", "Time Stamp(s)"))
